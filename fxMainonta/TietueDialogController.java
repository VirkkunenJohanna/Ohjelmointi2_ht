...
public class TietueDialogController<TYPE extends Tietue> implements ModalControllerInterface<TYPE>,Initializable  {
    ...
    @FXML private void handleOK() {
        // tässä käydään läpi kaikki valitun tietueen (eli tässä tapauksessa Henkilön) kentät niin monta kuin niitä on
        for (int i = tietueKohdalla.ekaKentta(); i < tietueKohdalla.getKenttia(); i++) {
            boolean onkoVirhe = edits[i].getStyleClass().contains("virhe"); // tässä haetaan vuorotellen jokaisen kentän CSS-luokkien listaus ja tsekataan, että löytyykö sieltä luokka nimeltä "virhe"
            if (onkoVirhe == true) { // tänne mennään, jos kentässä on päällä CSS-tiedostosta "virhe" eli ts. kentän tausta on punainen
                naytaVirhe("Korjaa virheelliset tiedot");
                return;
            }
        }
